...
BASE_DIR = os.path.dirname(os.path.abspath(__file__))# 获取当前文件的绝对路径，并转换为目录路径
ROOT_DIR = BASE_DIR# 将根目录设置为当前文件所在的目录
sys.path.append(os.path.join(ROOT_DIR, 'models'))# 将models目录添加到系统路径中，以便可以导入该目录下的模块
# 定义一个字典，用于存储不同类别物体对应的分割标签
#seg_classes = {'Earphone': [16, 17, 18], 'Motorbike': [30, 31, 32, 33, 34, 35], 'Rocket': [41, 42, 43], 'Car': [8, 9, 10, 11], 'Laptop': [28, 29], 'Cap': [6, 7], 'Skateboard': [44, 45, 46], 'Mug': [36, 37], 'Guitar': [19, 20, 21], 'Bag': [4, 5], 'Lamp': [24, 25, 26, 27], 'Table': [47, 48, 49], 'Airplane': [0, 1, 2, 3], 'Pistol': [38, 39, 40], 'Chair': [12, 13, 14, 15], 'Knife': [22, 23]}
seg_classes = {'cotton': [0,1,2]}
# 创建一个空字典，用于存储分割标签到类别的映射关系
#seg_label_to_cat = {} # {0:Airplane, 1:Airplane, ...49:Table}
seg_label_to_cat = {0:'cotton',1:'cotton',2:'cotton'}
# 遍历所有类别及其对应的分割标签
for cat in seg_classes.keys():
    for label in seg_classes[cat]:
        seg_label_to_cat[label] = cat# 将分割标签映射到对应的类别

def to_categorical(y, num_classes):
    """ 1-hot encodes a tensor """
    new_y = torch.eye(num_classes)[y.cpu().data.numpy(),]# 使用torch.eye创建一个单位矩阵，并根据y中的类别标签选择对应的行
    if (y.is_cuda):# 如果输入张量在GPU上，则将结果转移到GPU上
        return new_y.cuda()
    return new_y
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=5, help='batch size in testing [default: 24]')

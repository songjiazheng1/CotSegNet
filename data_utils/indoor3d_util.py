...
DATA_PATH = os.path.join(ROOT_DIR, 'data','s3dis', 'Stanford3dDataset_v1.2_Aligned_Version')# 设置数据集的路径
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]# 读取'meta/class_names.txt'文件中的每一行，并去除行尾的空白字符
g_class2label = {cls: i for i,cls in enumerate(g_classes)}# 创建一个字典，将类别名映射到对应的索引
g_class2color = {'ceiling':	[0,255,0],
                 ...
                 'bookcase':    [10,200,100],
                 'board':       [200,200,200],
                 'clutter':     [50,50,50]}  # 定义每个类别对应的颜色
g_easy_view_labels = [7,8,9,10,11,1]# 定义容易识别的类别标签
g_label2color = {g_classes.index(cls): g_class2color[cls] for cls in g_classes}# 创建一个字典，将类别名映射到对应的颜色
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    """
    points_list = []# 初始化一个空列表，用于存储所有点和标签
    for f in glob.glob(os.path.join(anno_path, '*.txt')):# 遍历注释文件夹中的所有txt文件
        cls = os.path.basename(f).split('_')[0]# 获取文件名并提取类别名
        print(f)# 打印当前处理的文件名
        if cls not in g_classes: # note: in some room there is 'staris' class..# 如果类别名不在预定义的类别列表中
            cls = 'clutter'# 将类别名设为 'clutter' (杂物)

        points = np.loadtxt(f)# 读取点数据
        labels = np.ones((points.shape[0],1)) * g_class2label[cls]# 生成对应的标签，标签值根据类别名映射
        # 根据class_names.txt 里的类别从0开始排序 ，标签0代表txt里面的第一个类别，1代表第二个
        points_list.append(np.concatenate([points, labels], 1)) # Nx7# 将点数据和标签合并，并添加到列表中
    
    data_label = np.concatenate(points_list, 0)# 将所有点数据和标签合并成一个数组
    xyz_min = np.amin(data_label, axis=0)[0:3]# 计算每个维度的最小值
    data_label[:, 0:3] -= xyz_min# 将所有点的数据平移，使最小值变为原点
    
    if file_format=='txt':# 如果文件格式是txt
        fout = open(out_filename, 'w')# 打开输出文件
        for i in range(data_label.shape[0]):# 遍历所有点
            fout.write('%f %f %f %d %d %d %d\n' % \
                          (data_label[i,0], data_label[i,1], data_label[i,2],
                           data_label[i,3], data_label[i,4], data_label[i,5],
                           data_label[i,6]))# 将点数据和标签写入文件
        fout.close()# 关闭文件
    elif file_format=='numpy':# 如果文件格式是numpy
        np.save(out_filename, data_label)# 保存为numpy文件
    else:# 如果文件格式未知
